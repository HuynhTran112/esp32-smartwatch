...
static void lvgl_port_touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (err != ESP_OK) {
        ...
        if (now - last_err_time > 2000) {
            ESP_LOGE("TOUCH_DEBUG", "esp_lcd_touch_read_data failed: 0x%x", err);
            last_err_time = now;
        }
    }
    ...
    if (touchpad_pressed && touchpad_cnt > 0) {
        ...
        // ESP_LOGI("TOUCH_DEBUG", "Touch active! X:%d Y:%d RawX:%d RawY:%d", 
        //          (int)data->point.x, (int)data->point.y, (int)touchpad_x[0], (int)touchpad_y[0]);
    } else {
